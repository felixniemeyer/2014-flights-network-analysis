...
We can also appreciate these major differences by analysing various parameters of the network.
...
That is the reason why the geometric distance diameters of these airline networks are easily over 12000 kms.
...
And as expected, the diameter of these airline networks are below 4000 kms. 
...
Full Service Carriers show an average route distance of around 2300 kms, while the Low Cost Carriers show that their average route distance is about 1400 kms, which is significantly lower.
...
But when we look at the mean, they are only around 1400 kms, which represent airlines like RyanAir or Southwest Airlines.
This discrepancy can be explained by analysing the median geometric distance of these networks.
These Chinese Airlines have a median of around 1100 kms, which means they have a lot of routes which are within China region only. 
And looking at the full network, we can also say that, the main reason their diameter is over 12000 kms is because they had recently started to offer few routes to countries like Canada, United States and Germany.
...
And of course, their routes stop at a maximum distance of around 4000 kms. 
Looking at the Full Service Carriers, though the global peak of the distribution of United and Lufthansa is also around the same distance of 1000 kms, we can see that there is a second small peak at around 6000 kms. This is nothing but the average route distance for Transatlantic flights between United States and Europe. 
...
The main metric of our analysis $\Largeness_{2}$, is currently dependent on Patronages in each airport and the routes available for each airline. Though it already results in providing results which are close to the real world, we can still improve it with few changes in the metric or data. With access to the exact population data for each airport, the results would be more accurate.

Another way to improve the metric, is bringing in the centrality of each airport as a weighting factor in the analysis, especially the eigenvector centrality. 
Unlike degree centrality, which only looks at the number of connections or neighbours for one particular airport, eigenvector centrality looks at both the number, and the centrality of the neighbours for each airport.\cite{eigenvector_centrality}
There is a not insignificant differences between the patronages of each airport and the eigenvector centrality of each airport in our data, as shown in Figure~\ref{fig:patronage_eigenvector}.
...
\begin{figure}
	...
	\caption{
	Patronage vs Eigenvector Distrubution}
